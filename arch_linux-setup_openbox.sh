#!/bin/bash
# This script cleans up and configures an Arch Linux Openbox install that was installed with "archinstall." Run as a normal user!
...
sudo pacman -S thunar thunar-archive-plugin thunar-media-tags-plugin thunar-shares-plugin 	thunar-vcs-plugin thunar-volman tumbler --noconfirm

# Install tint2 panel.
sudo pacman -S tint2 --noconfirm

# Misc. Openbox stuff.
sudo pacman -S 	lxappearance-obconf-gtk3 menumaker obconf-qt

# Install Alacritty and copy over custom config file.
mkdir -p "$HOME"/.config/alacritty
sudo mkdir -p /etc/skel/.config/alacritty
sudo pacman -S alacritty --noconfirm
cp -v "$HOME"/linux-stuff/Dotfiles/config/alacritty/alacritty.toml "$HOME"/.config/alacritty/alacritty.toml
sudo cp -v "$HOME"/linux-stuff/Dotfiles/config/alacritty/alacritty.toml /etc/skel/.config/alacritty/alacritty.toml

# Install icon and theme.
echo -e "${BLUE}Installing Papirus icon theme and Arc GTK/Kvantum theme...${NC}"
paru -S papirus-icon-theme kvantum-theme-arc arc-solid-gtk-theme kvantum kvantum-qt5 qt5ct --noconfirm
sleep 10 ; clear
...
echo -e "${BLUE}Installing power-profiles-daemon package...${NC}"
...
sudo pacman -S unrar vlc transmission-qt pinta audacity k3b okular spectacle p7zip clipgrab --noconfirm
sleep 10 ; clear
